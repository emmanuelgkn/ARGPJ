...
def grid_search():
    ...
    nbeps_values = [ 50, 100] 
    target_update_freq = [20,50]   


    results = []

    combinations = list(itertools.product(batch_sizes, learning_rates, nbeps_values, target_update_freq))
    total_combinations = len(combinations)

    for idx, (batch_size, lr, nbeps, target) in enumerate(combinations):
        print(f"\n=== Testing configuration {idx+1}/{total_combinations} ===")
        print(f"Batch size: {batch_size}, Learning rate: {lr}, NBEps: {nbeps}, Target update freq: {target}")
